          ...
          <foreignObject v-for="(app, idx) in leftApps" :key="'left-' + app.name"
            ...
            <div xmlns="http://www.w3.org/1999/xhtml" 
              ...
              <div class="w-7 h-7 rounded-lg bg-surface-container flex items-center justify-center p-1 shadow-inner shrink-0 overflow-hidden">
                <img v-if="app.logo" :src="app.logo" :alt="app.name" class="w-5 h-5 object-contain" />
        ...
        <div class="w-full">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div v-for="app in leftApps" :key="app.name" class="glass-card p-4 rounded-xl border border-border-subtle flex items-center gap-3">
              <img v-if="app.logo" :src="app.logo" :alt="app.name" class="w-5 h-5 object-contain shrink-0" />
...
const leftApps = [
  { name: 'WooCommerce', icon: 'shopping_bag', logo: 'https://upload.wikimedia.org/wikipedia/commons/2/2a/WooCommerce_logo.svg', trigger: 'New Order placed', detail: 'Triggers instant order verification, bills pdf, and tracking details to client WhatsApp.' },
  ...
  { name: 'Razorpay Gateway', icon: 'credit_card', logo: 'https://upload.wikimedia.org/wikipedia/commons/b/b5/Razorpay_logo.svg', trigger: 'Payment Received', detail: 'Verifies transaction status and pushes interactive invoice receipt to customer on WhatsApp.' },
  { name: 'Google Forms', icon: 'description', logo: 'https://upload.wikimedia.org/wikipedia/commons/2/2f/Google_Forms_2020_Logo.svg', trigger: 'Response submitted', detail: 'Maps form entries, triggers confirmation, and books meeting dates automatically.' },
...
const rightApps = [
  ...
  { name: 'Zoho CRM', icon: 'hub', logo: 'https://upload.wikimedia.org/wikipedia/commons/e/e1/Zoho_logo.svg', action: 'Create Sales Contact', detail: 'Syncs lead status and transcripts, assigning qualified contacts to active salespeople.' },
  { name: 'ActiveCampaign', icon: 'campaign', logo: 'https://upload.wikimedia.org/wikipedia/commons/1/1a/ActiveCampaign_logo.svg', action: 'Sync Newsletter tag', detail: 'Updates drip campaigns and email list status depending on WhatsApp conversation intent.' },
